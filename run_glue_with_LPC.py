...
def train(args, reg_params, train_dataset, new_model, pretrained_model, tokenizer, num_labels):
    """ Train the model """
    ...
    if args.optimizer == 'Adam':
    ...
    elif args.optimizer == 'LPC':
        # Prepare for the grouped parameters for LPC optimizer.
        ...
        optimizer = LPC(optimizer_grouped_parameters, reg_lambda=args.reg_lambda, lr=args.learning_rate, eps=args.adam_epsilon,
                            anneal_fun=args.lpc_anneal_fun, anneal_k=args.lpc_anneal_k,
                            anneal_t0=args.lpc_anneal_t0, pretrain_cof=args.lpc_pretrain_cof)
    else:
        ValueError
    ...
    if args.fp16:
        ...
        new_model, optimizer = amp.initialize(new_model, optimizer, opt_level=args.fp16_opt_level)
    ...
    if args.local_rank != -1:
        new_model = torch.nn.parallel.DistributedDataParallel(
            new_model, device_ids=[args.local_rank], output_device=args.local_rank, find_unused_parameters=True,
        )
    ...
    tr_loss, logging_loss, train_logging_loss = 0.0, 0.0, 0.0
    ...
    for epoch_num in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            outputs_n = new_model(**inputs)
            outputs_b = pretrained_model(**inputs)
            logits_n = outputs_n[1]
            logits_b = outputs_b[1]
            ...
            if inputs['labels'] is not None:
                ...
                if args.output_mode == "regression":
                    loss_fct = MSELCLoss()
                    if num_labels == 1:
                        loss = loss_fct(logits_n.squeeze(), logits_b.squeeze(), inputs['labels'].squeeze(), args.logits_calibraion_degree)
                    else:
                        loss = loss_fct(logits_n, logits_b, inputs['labels'])
                elif args.output_mode == "classification":
                    loss_fct = CELCLoss()
                    loss = loss_fct(logits_n.view(-1, num_labels), logits_b.view(-1, num_labels), inputs['labels'].view(-1), args.logits_calibraion_degree)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # Calculate the Euclidean distance between the weights of the pretrained model and fine-tuning model
                if args.logging_Euclid_dist:
                    dist = torch.sum(torch.abs(torch.cat(
                        [p.view(-1) for n, p in new_model.named_parameters() if args.model_type in n]) - torch.cat(
                        [p.view(-1) for n, p in pretrained_model.named_parameters() if
                         args.model_type in n])) ** 2).item()
                else:
                    dist = 0.0
                dist_sum += dist
                dist_num += 1
                ...
                if args.fp16:
                    ...
                    torch.nn.utils.clip_grad_norm_(new_model.parameters(), args.max_grad_norm)

                optimizer.step(reg_params)
                ...
                if args.local_rank in [-1, 0] \
                    ...
                    anneal_lambda = anneal_function(args.lpc_anneal_fun, global_step, args.lpc_anneal_k,
                                                    args.lpc_anneal_t0, args.lpc_anneal_w)
                    logger.info(
                ...
                if args.local_rank in [-1, 0] \
                        and args.eval_logging_steps > 0 and global_step % args.eval_logging_steps == 0:
                    logs = {}
                    if (
                        args.local_rank == -1 and args.evaluate_during_training
                    ):  # Only evaluate when single GPU otherwise metrics may not average well
                        results = evaluate(args, new_model, tokenizer)
                    ...
                    logger.info('eval results: {}'.format({**logs, **{"step": global_step}}))
                    eval_acc = logs['eval_' + glue_tasks_acc_keys[args.task_name]]
                    dev_best = max(dev_best, eval_acc)
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    model_to_save = (
                        new_model.module if hasattr(new_model, "module") else new_model
                    ...
                    scheduler_state_dict.pop('lr_lambdas')
        ...
        # update omega after update_epoch epochs
        if epoch_num % args.update_epoch == 0:
            optimizer_ft = LPC_omega_update(reg_params)
            new_model, reg_params = compute_omega_grads_norm(args, new_model, reg_params, train_dataloader, optimizer_ft)
            new_model.eval()

            if epoch_num > 0:
                new_model, reg_params = consolidate_reg_params(new_model, reg_params)

    if args.local_rank in [-1, 0]:
        tb_writer.close()

    logger.info('Best dev results: {}'.format(dev_best))
...
def main():
    ...
    if args.model_type == "albert":
        config = config_class.from_pretrained(
            ...
            attention_probs_dropout_prob=args.albert_dropout,
        )
    else:
        config = config_class.from_pretrained(
            args.config_name if args.config_name else args.model_name_or_path,
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, reg_params, train_dataset, new_model, pretrained_model, tokenizer, num_labels)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        checkpoints = [args.output_dir + '/checkpoint-4500']
        ...
        for checkpoint in checkpoints:
            ...
            new_model = model_class.from_pretrained(checkpoint)
            new_model.to(args.device)
